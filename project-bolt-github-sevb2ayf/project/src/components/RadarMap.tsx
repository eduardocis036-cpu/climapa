...
export default function RadarMap({ lat, lon, fullscreen, onToggleFullscreen }: Props) {
  ...
  // Shared layer content used in both card and fullscreen modes
  const mapLayers = (
    <>
      <TileLayer
  ...
  // F4: Only render ONE MapContainer at a time. When fullscreen is toggled,
  // the card MapContainer unmounts (returning null) and the fullscreen one mounts,
  // ensuring a single Leaflet instance lives in memory at any time.
  if (fullscreen) {
    return (
      <div
      ...
      >
        <div className="flex items-center justify-between p-4" onClick={(e) => e.stopPropagation()}>
          ...
          <button
            className="glass-card rounded-full p-2 text-white/80 hover:text-white"
            onClick={onToggleFullscreen}
          >
